...
namespace Algorithms.DataStructures
{
    public class Heap<T> : AlgorithmBase<T> where T : IComparable
    {
        ...
        private void Sort(int curentIndex, int maxLengh = -1)
        {
            int maxIndex = curentIndex;
            int leftIndex;
            int rightIndex;

            maxLengh = maxLengh == -1 ? Count : maxLengh;

            while (curentIndex < maxLengh)
            {
                leftIndex = 2 * curentIndex + 1;
                rightIndex = 2 * curentIndex + 2;

                if (leftIndex < maxLengh && Compare(Items[leftIndex], Items[maxIndex]) == 1)
                {
                    maxIndex = leftIndex;
                }

                if (rightIndex < maxLengh && Compare(Items[rightIndex], Items[maxIndex]) == 1)
                {
                    maxIndex = rightIndex;
                }
